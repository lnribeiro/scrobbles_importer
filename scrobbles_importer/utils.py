...
def get_last_page(user):
    last_page = requests.get(
        'http://www.last.fm/user/{}/library?page=99999'.format(user))
...
def get_scrobbles(user, start_page=1, end_page=5, verbose=False):
    ...
    for page_number in range(start_page, end_page + 1):
        ...
        path = 'http://www.last.fm/user/{}/library?page={}'.format(
            user, page_number)
        page = requests.get(path)
        tree = html.fromstring(page.content)

        # process DOM tree using xpath to extract the desired information
        scrobbles_artists = tree.xpath(
            '//span[@class="chartlist-artists"]/a/text()')
        scrobbles_songs = tree.xpath(
            '//span[@class="chartlist-ellipsis-wrap"]/a/text()')
        scrobbles_timestamps = tree.xpath(
            '//td[@class="chartlist-timestamp"]/span/text()')
